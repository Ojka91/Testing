...
public class UsuarioServiceSolvedImpl implements UsuarioService {

	private static final Logger LOG = Logger.getLogger(UsuarioServiceSolvedImpl.class.getName());
	...
	public UsuarioDto insertar(UsuarioDto dto) throws ServicioException {
		...
		try {
			...
			String mensaje = "Se ha producido un error con la conexion filenet";
			LOG.error(mensaje, e);
			throw new FilenetException(mensaje);
		}
		...
		try {
			...
			String mensaje = "Se ha producido un error al crear la carpeta del usuario";
			LOG.error(mensaje, e);
			throw new FilenetException(mensaje);
		} finally {
			try {
				...
				String mensaje = "Se ha producido un error al desconectar de filenet";
				LOG.error(mensaje, e);
				throw new FilenetException(mensaje);
			}
		}

		Map<String, ?> datosUsuario = obtenerDatosUsuario();
	...
	private Map<String, ?> obtenerDatosUsuario() {
		...
		return DatosAutenticacion.class.cast(authentication).getDatosAdicionales();
	}
}
